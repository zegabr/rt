...
vec3 color(const ray& r, const hitable *world, const camera &cam, const phongLight &light){
    hit_record rec;
    if(world->hit(r,0.00000001,FLT_MAX,rec)){ // se acertar algum objeto da imagem, entra nesse if
        hit_record h;
        if(world->hit(ray(rec.p, light.position-rec.p), 0.001,FLT_MAX, h)) {
...
int main(){
    ...
    list[0] = new sphere(vec3(0.0,1.0,-1.0),0.2, phongMaterial(vec3(0.0,0.0,1.0), 0.2, 0.5, 0.6, 1.0)); // esfera do centro
    ...
    phongLight light(vec3(1.0,1.0,1.0), vec3(2.0,1000.0,-1.0)); // 1 parametro é a cor, segundo é a posição
